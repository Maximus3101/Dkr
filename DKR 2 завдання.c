...
void swap_next_columns(int *arr, int column, const int matrix_size);
int once_sort_columns(int *sortable_matrix, const int matrix_size);
...
void sort(int *sortable_matrix, int *result_matrix, const int matrix_size)
{
    ...
    while (once_sort_columns(&result_matrix[0], matrix_size))
        ;
}
...
int once_sort_columns(int *sortable_matrix, const int matrix_size)
{
    ...
    for (int i = 0; i < matrix_size; i++)
        sum1 += sortable_matrix[i * matrix_size];
    for (int j = 1; j < matrix_size; j++)
    {
        ...
        if (sum2 < sum1)
        {
            swap_next_columns(&sortable_matrix[0], j, matrix_size);
...
{
    for (int i = 0; i < matrix_size; i++)
        swap(&arr[i * matrix_size + (column - 1)], &arr[i * matrix_size + column]);
}
